...
(define (get-vertices G)
  (if (eq? G empty) '()
      (usort (cons (car (car G)) (cons (car (cdr (car G))) (get-vertices (cdr G)))))
             )
  )
...
(define (degree G v)
  (if (eq? G '()) 0
      (if (and (eq? v (car (car G))) (eq? v (car (cdr (car G))))) (+ 2 (degree (cdr G) v))
          (if (or (eq? v (car (car G))) (eq? v (car (cdr (car G))))) (+ 1 (degree (cdr G) v))
              (+ 0 (degree (cdr G) v))
              )
          )
      )
...
(define (all-even? G)
  (all-even?help G (get-vertices G))
  )
(define (all-even?help G v)
  (if (eq? v '()) #t
      (if (eq? (modulo (degree G (car v)) 2) 0) (all-even?help G (cdr v))
          #f
          )
      )
  )
...
(define (adjacent G v)
  (if (eq? G empty) '()
      (if (eq? (car (car G)) v) (usort (cons (car (cdr (car G))) (adjacent (cdr G) v)))
          (if (eq? (car (cdr (car G))) v) (usort (cons (car (car G)) (adjacent (cdr G) v)))
              (adjacent (cdr G) v)
              )
          )
      )
...
(define (connected? G)
  (if (eq? G '()) #t
      (if (equal? (usort (dfs G '() (car (car G)))) (usort (get-vertices G))) #t
          #f
          )
      )
  )
...
(define (eulerian? G)
  (if (and (connected? G) (all-even? G)) #t
      #f
      )
  )
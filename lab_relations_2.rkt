...
(define (compose RO RI)
  (make-set(composeHelp RO RI RO))
  )
(define (composeHelp RODup RI RO)
  (if (eq? RI '()) '()
      (if (eq? RO '()) (composeHelp RODup (cdr RI) RODup)
          (if (eq? (car (car RO)) (car (cdr (car RI)))) (cons (list (car (car RI)) (car (cdr (car RO)))) (composeHelp RODup RI (cdr RO)))
                  (composeHelp RODup RI (cdr RO))
              )
          )
      )
  )
...
(define (power R k)
  (if (eq? k 0) '()
  (make-set (powerHelp R k))
  )
  )
(define (powerHelp R K)
  (if (eq? K 1) R
      (compose R (powerHelp R (- K 1)))
      )
  )
...
(define (transitive-closure R)
  (make-set(transCloseHelp R (length R))
  )
  )
(define (transCloseHelp R k)
  (if (eq? k 0) R
          (union (transCloseHelp R (- k 1)) (power R k))
              )
          )
...
(define (transitive? R)
  (if (set-equal? R (transitive-closure R)) #t
  #f
  )
 )
...
(define (EQ-relation? R n)
  (if (and (reflexive? R n) (and (symmetric? R) (transitive? R))) #t
      #f
      )
  )

...
(define (reflexive? R n)
  (if (eq? R '()) #f
      (if (and (eq? (car (cdr ( car R))) n) (eq? (car (car R)) n)) #t
      (reflexive? (cdr R) n))
      )
  )
...
(define (reflexive-closure R n)
  (cons (id n) (reflexive-closureHelper R n 1)))
(define (reflexive-closureHelper R n m)
  (if (eq? m (+ n 1)) '()
   (if (reflexive? R m) (reflexive-closureHelper (cdr R) (+ m 1))
   R )
     )
  )
...
(define (inverse R)
  (if (eq? R '()) '()
      (cons (list (car (cdr (car R))) (car (car R))) (inverse (cdr R)))
      )
  )
...
(define (symmetric? R)
  (if (eq? R '()) #t
      (if (element? '((car (cdr (car R))) (car (car R))) R) (symmetric? (cdr R))
      #f)
      )
  )
...
(define (symmetric-closure R)
  (cons R symmetric-closure R)
  )
(define (symmetric-closurehelp R)
  (if (eq? R '()) '()
   (if (element? '((car (cdr (car R))) (car (car R))) R) (symmetric-closurehelp (cdr R))
   (cons '((car (cdr (car R))) (car (car R))) (symmetric-closurehelp (cdr R))))
      )
  )
...
(define (relates-to v R)
  (if (eq? R '()) '()
      (if (eq? v (car (cdr (cdr R)))) (cons (car (cdr (cdr R))) (relates-to v (cdr R)))
           (relates-to v (cdr R))
         )
    )
)

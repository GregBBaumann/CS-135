...
(define (generate p q e)
  (list (list (* p q) e)
  (list (* p q) (mul-inv e (lcm (- p 1) (- q 1)))))
  )
...
(define (mod-exp b e m)
  (if (eq? e 0) 1
      (if (eq? (modulo e 2) 0) (modulo (expt (mod-exp b (/ e 2) m) 2) m)
          (modulo (* (mod-exp b (- e 1) m) b) m)
          )
      )
  )
...
(define (encrypt plaintext key)
  (encrypthelp (string->list plaintext) key)
  )
(define (encrypthelp L key)
  (if (eq? L '()) '()
      (cons (mod-exp (char->integer (car L)) (car (cdr key)) (car key)) (encrypthelp (cdr L) key))
      )
  )
...
(define (decrypt ciphertext key)
  (list->string (decrypthelp ciphertext key))
  )
(define (decrypthelp L key)
  (if (eq? L '()) '()
      (cons (integer->char (mod-exp (car L) (car (cdr key)) (car key))) (decrypthelp (cdr L) key))
      )
  )

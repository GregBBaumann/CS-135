...
;; Pledge: I pledge my honor I ave abided by the stevens honor system
;;-------------------------------------------------------------------------------
...
(define (pred p)
  (if (eq? z p) (eopl:error "pred error: zero has no predecessor!")
      (car p)
      )
  )
...
;; Type signature: (p- peano peano) -> peano
(define (p- a b)
  (if (and (not (pz? b)) (pz? a)) (eopl:error "subtraction error: a < b!")
      (if (pz? b) a
          (p- (pred a) (pred b))
          )
      )
...
;; Type signature: (p% peano peano) -> peano
(define (p% a b)
  (if (pz? b) (eopl:error "modulo error: mod is zero!")      
      (if (p> b a) a
          (p% (p- a b) b)
          )
      )
...
(define (p-prime? p)
  (p-prime?help p (itp 2)))
(define (p-prime?help p r)
  (if (eq? p r) #t
      (if (pz? (p% p r)) #f
          (p-prime?help p (succ r))
          )
      )
  )

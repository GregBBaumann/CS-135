...
(define (indegree Es v)
  (if (eq? Es '()) 0
      (if (eq? v (car (cdr (car Es)))) (+ 1 (indegree (cdr Es) v))
          (indegree (cdr Es) v)
          )
      )
  )
...
(define (find-top G)
  (find-tophelp G (get-vertices G))
  )
(define (find-tophelp G v)
  (if (eq? v '()) (tsort-error)
      (if (equal? (indegree (get-edges G) (car v)) 0) (car v)
          (find-tophelp G (cdr v))
          )
      )
  )
...
(define (pop-vertex G v)
  (cons (popv-vertices (get-vertices G) v) (list (popv-edges (get-edges G) v)))
  )
...
(define (tsort G)
  (if (equal? G empty)'()
      (cons (find-top G) (tsort (pop-vertex G (find-top G))))
      )
  )

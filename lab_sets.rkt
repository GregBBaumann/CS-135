...
;; Pledge: I pledge my hononr I have abided by the stevens honor system
;;-------------------------------------------------------------------------------
...
(define (element? e L)
  (if (eq? e (car L)) #t (if (eq? L '()) #f (element? e (cdr L)))))
...
(define (unionHelp LA LB set)
  (if (eq? LA '()) (if (eq? LB '()) set (if (element? (car LB) set)  (unionHelp LA (cdr LB) (cons set (car LB))) (unionHelp LA (cdr LB) set)))
      (if (element? (car LA) set)  (unionHelp (cdr LA) LB (cons set (car LA))) (unionHelp (cdr LA) LB set))))
...
(define (intersection LA LB)
  intersectionHelp LA LB '()
)

(define (intersectionHelp LA LB set)
  (if (eq? LA '()) set
      (if (element? (car LA) set)  (unionHelp (cdr LA) LB set)
          (if (element? (car LA) LB) (intersectionHelp (cdr LA) LB (cons set (car LA)))
              (intersectionHelp (cdr LA) LB set))))
)
...
(define (subset? SA SB)
  (if (eq? SA '()) #t
      (if (element? (car SA) SB) (subset? (cdr SA) SB)
          #f))
)
...
(define (set-equal? SA SB)
  (if (eq? SA '()) #t
      (if (element? (car SA) SB) (set-equal? (cdr SA) SB)
          #f))
...
(define (set-difference LA LB)
  set-differenceHelp LA LB '())
(define (set-differenceHelp LA LB set)
  (if (eq? LA '()) set
      (if (element? (car LA) LB)  (set-differenceHelp (cdr LA) LB set)
          (if (element? (car LA) set) (set-differenceHelp (cdr LA) LB set)
              (set-differenceHelp (cdr LA) LB (cons set (car LA))))))
)
...
(define (sym-diff LA LB)
  cons (set-difference LA LB) (set-difference LB LA))
...
(define (cardinality L)
  cardinalityHelp L 0 '())
(define (cardinalityHelp L n set)
  (if (eq? L '()) n
      (if (element? (car L) set) (cardinalityHelp (cdr L) n set)
          (cardinalityHelp (cdr L) (+ n 1) (cons set (car L)))))
)
...
(define (superset? SA SB)
  (if (eq? SB '()) #t
      (if (element? (car SB) SA) (superset? SA (cdr SB))
          #f))
)
